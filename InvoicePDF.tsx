      ...
      <>
          ...
          {financialDetails.status === 'First Half Payment Invoice' && (
            <>
              <View style={[classes.row, classes.line]}>
                ...
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{financialDetails.balanceOwed}</Text>
                </View>
              </View>
              <View style={classes.rowTight}>
                <Text style={classes.cellTight}>{translations.balanceDue}</Text>
        ...
        <Text style={classes.details}>
          ...
          {extraNotes.includedServices.length > 0 && (
            <>
              {' '}
              {translations.guestNotes.servicesInfo} {extraNotes.includedServices.join(', ')}
              {'.'}
            </>
          )}
        </Text>
        {extraNotes.otherRules !== '' && <Text style={classes.details}>{extraNotes.otherRules}</Text>}
        {extraNotes.knownThings !== '' && <Text style={classes.details}>{extraNotes.knownThings}</Text>}
        <Text style={classes.details}>{translations.guestNotes.overCapacity1}</Text>
        <Text style={classes.details}>
          {translations.guestNotes.overCapacity2}
          {destination.guestNumber}
          {translations.guestNotes.overCapacity3}
        </Text>
        <Text style={classes.details}>{translations.guestNotes.overCapacity4}</Text>
    ...
    return (
      <>
        ...
        <View style={classes.tableColumn}>
          ...
          <View style={classes.row}>
            <Text style={[classes.cell, classes.black]}>{translations.securityDeposit}:</Text>
            <View style={classes.right}>
              <Text style={classes.cellFlex}>{reservation.securityDeposit}</Text>
            </View>
          </View>
          {reservation.photographyDiscountValue !== undefined && (
            <View style={classes.row}>
              <Text style={[classes.cell, classes.black]}>{translations.photographyDiscount}:</Text>
              <View style={classes.right}>
                <Text style={classes.cellFlex}>{reservation.photographyDiscountValue}</Text>
              </View>
            </View>
          )}
          <View style={[classes.row, classes.line]}>
            <Text style={[classes.cell, classes.black]}>{translations.youEarn}</Text>
            <View style={classes.right}>
          ...
          {reservation.changeType && (
            <>
              <View style={[classes.row, classes.line]}>
                ...
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>CLP {reservation.changeType}</Text>
                </View>
              </View>
            </>
          )}
        ...
        <Text style={classes.detailsHead}>{translations.notes}</Text>
        <Text style={classes.details}>{translations.ownerNotes.securityDepositInfo}</Text>
        <Text style={classes.details}>{translations.ownerNotes.basicServices}</Text>
        ...
        <Text style={classes.details}>{translations.ownerNotes.cancellationPolicy}</Text>
        <Text style={classes.details}>{translations.ownerNotes.roomDistribution}</Text>
      </>
    )
  }
    ...
    <Document>
      <Page size="A4" style={classes.page}>
        <View style={classes.section}>
          ...
          <Text style={classes.sectionTitle}>{translations.bookingSummary}</Text>

          <View style={classes.tableNoMargin}>
            <View style={classes.subTable}>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.guestName}</Text>
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{reservation.userName}</Text>
                </View>
              </View>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.propertyName}</Text>
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{reservation.destinationName}</Text>
                </View>
              </View>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.destination}</Text>
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{destination.city}</Text>
                </View>
              </View>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.country}</Text>
                <View style={classes.right}>
              ...
              {reservation.nationality && (
                <View style={classes.subTableRow}>
                  <Text style={[classes.cell, classes.black]}>{translations.nationality}</Text>
                  <View style={classes.right}>
                    <Text style={classes.cellFlex}>{reservation.nationality}</Text>
                  </View>
                </View>
              )}
            </View>
            <View style={classes.subTable}>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.arrivalDate}</Text>
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{reservation.checkIn}</Text>
                </View>
              </View>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.departureDate}</Text>
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{reservation.checkOut}</Text>
                </View>
              </View>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.totalNights}</Text>
                <View style={classes.right}>
                  <Text style={classes.cellFlex}>{reservation.nights}</Text>
                </View>
              </View>
              <View style={classes.subTableRow}>
                <Text style={[classes.cell, classes.black]}>{translations.cancellationPolicy}</Text>
                <View style={classes.right}>
                  <Link
              ...
              {reservation.groupType && (
                <View style={classes.subTableRow}>
                  <Text style={[classes.cell, classes.black]}>{translations.groupType}</Text>
                  <View style={classes.right}>
          ...
          <View style={classes.tableColumn}>
            <View style={classes.row}>
              <Text style={[classes.cell, classes.black]}>{translations.cost}</Text>
              <Text style={[classes.cell, classes.black, classes.center]}>{translations.detail}</Text>
              <View style={classes.right}>
                <Text style={[classes.cellFlex, classes.black, classes.smallPad]}>{translations.price}</Text>
              </View>
            </View>
            {costs.map((cost, index) => (
              <View key={index} style={[classes.row, classes.line]}>
                <Text style={classes.cell}>{cost.name}</Text>
                <Text style={[classes.cell, classes.center]}>{cost.detail}</Text>
                <View style={classes.right}>
          ...
          <>
            ...
            <Text style={classes.detailsHead}>Política de Cancelación | ES:</Text>
            <View style={classes.details}>
